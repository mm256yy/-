<template>
	<view class="content">
		...
		<view class="payBox">
			<image :src="payImg" style="width: 360rpx;height: 360rpx;" mode="aspectFill">
			</image>
			<view class="successText">恭喜您支付成功!</view>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			self.id = JSONBig.parse(e.id).toString();
			self.getImg();
		},
		onShow() {
			this.getImg();
		...
		methods: {
			Topath() {
				uni.redirectTo({
					url: '/pages/order/orderDetail?id=' + this.id
				})
			},
			Tohome() {
				uni.switchTab({
			...
			getImg() {
				...
				T2UZ8Z79OFNYZ962F85XH`).then(res => {
					if (res.code == 200) {
						...
						let url= list.filter(itm => itm.type === 'paysuccess');
