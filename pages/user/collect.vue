	...
	<view class="content">
		<view class="goods_box">
			<view class="goods_list_box">
				<u-swipe-action v-for="(item,index) in collectList" :key="item.id" >
				<!-- <view class="goods" v-for="(item, index) in collectList" :key="item.id" > -->
					...
					<view class="goods">
						<view class="box5"  @click="Todetail(item.dataId)">
							<view class="image">
								<image class="goodsImg" :src="item.extJSON.previewUrl" mode="aspectFill"></image>
							</view>
							<view class="info">
								<view class="title">{{item.extJSON.goodsName}}</view>
								<view class="box">
									<view class="price">
										<text class="price-text">价格:</text><text class="mini_price">¥{{item.extJSON.goodsPrice}}</text>
									</view>
									<view class="add price-text">
										<view>{{timestampToTime2(item.extJSON.createTime)}}收藏</view>
									</view>
								</view>
							</view>
						</view>
...
<script>
	...
	export default{
		...
		onShow() {
			let self=this;
			self.getbussinessId()
			console.log(this.collectList,'测测试数')
		},
		methods:{
			...
			getcollectList(){
				...
				self.Get('/api/plugs/searchPlugsCollectByUserId',postdata).then(res=>{
					if(res.code==200){
						...
						console.log(self.collectList.length,self.collectList,'123123123')
